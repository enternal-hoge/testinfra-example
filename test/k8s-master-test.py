...
def test_k8s_namespace_exist(host):
    assert k8s.is_namespace_exist('kube-system')

def test_k8s_pod_exist(host):
    assert k8s.is_pod_exist('kube-proxy-')
    assert k8s.is_pod_exist('kube-dns-v20-')
    assert k8s.is_pod_exist('kube-apiserver-')
    assert k8s.is_pod_exist('kube-controller-manager-')
    assert k8s.is_pod_exist('kube-scheduler-')
    assert k8s.is_pod_exist('kube-controller-manager-')
    assert k8s.is_pod_exist('kubernetes-dashboard-')
    assert k8s.is_pod_exist('heapster-')
    assert k8s.is_pod_exist('tiller-deploy-')

def test_istio_pod_exist(host):
    assert k8s.is_pod_exist('istio-ca-')
    assert k8s.is_pod_exist('istio-egress-')
    assert k8s.is_pod_exist('istio-ingress-')
    assert k8s.is_pod_exist('istio-mixer-')
    assert k8s.is_pod_exist('istio-pilot-')